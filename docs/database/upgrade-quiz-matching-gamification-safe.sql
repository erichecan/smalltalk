-- Quiz与Matching游戏化学习功能数据库升级脚本（安全版本）
-- 创建时间：2025-01-30 09:30:00
-- 说明：分步骤执行，包含错误处理，支持游戏化学习、积分系统、成就系统、打卡系统等功能
...
SELECT 'Step 14.1 完成: 成就数据插入成功' as result;
...
SELECT 'Step 14.2 完成: 主题数据插入成功' as result;
...
SELECT '可通过以下命令查看表详情: \d table_name' as detail_tip;
...
WHERE tablename IN (
    'user_learning_profiles', 'game_sessions', 'word_mastery_records', 
    'user_points', 'points_transactions', 'check_in_records', 
    'achievements', 'user_achievements', 'learning_topics', 
    'user_unlocks', 'practice_questions'
); 